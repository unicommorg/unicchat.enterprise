...
Публичный сервис компании Unicomm. Подключение к нему необходимо для работы аудио- и видеоконференций, а также аудиозвонков.
Расположен во внешнем периметре на серверах компании. Серверу UnicChat требуются исходящие соединения к этому сервису и не требуются входящие соединения.
...
> **Обязательный шаг. Запросите лицензию продукта UnicChat в компании Unicomm до начала установки.**
>
> Лицензируется продукт UnicChat как одна логическая единица. Сервисы установки, в том числе Solid Core, отдельной лицензией не покрываются.
>
...
Перед установкой нужна действующая лицензия продукта UnicChat (раздел 1.2).
...
Действующая лицензия продукта UnicChat от Unicomm (п. 1.2). Сервисы установки отдельно не лицензируются.
...
Nginx читает `fullchain.pem` и `privkey.pem` из `./certs/config/live/<домен>/`. Пути задают `SSL_CERT`, `SSL_KEY`, `DOCUMENTSERVER_SSL_*` и `MINIO_SSL_*`. Каталог `./certs` монтируется в `/certs`.

**Let's Encrypt на этом сервере.** Модель по умолчанию. DNS уже указывает на этот сервер, порты 80 и 443 свободны. Три сертификата, по одному на домен из `.env`.
...
**Свои файлы.** Положите уже выпущенный сертификат в те же каталоги: `certs/config/live/<домен>/fullchain.pem` и `privkey.pem` для каждого из трёх доменов. Пути в `.env` не меняют схему. Certbot не запускайте. Продление остаётся на стороне того, кто выпустил сертификат.

**Свой nginx на хосте.** Контейнер nginx не запускайте. Проксируйте на AppServer `:8080`, DocumentServer `:8880`, MinIO `:9000`. Примеры — `multi-server-install/nginx/examples/host/`. Сертификат живёт в конфигурации хостового nginx. Переменные `SSL_*` контейнера не используются.

**Внешний балансировщик.** Порт 443 завершает TLS до этой машины. Контейнерный nginx и Certbot не нужны, если пользователи не обращаются к портам этого хоста.
...
- 7881/TCP, 7882/UDP
- (50000-60000)/UDP (диапазон портов может быть изменён при развёртывании лицензионной версии)
...
Tasker читает секрет `KBTConfigs` из Vault. Секрет один раз создаёт `vault-init` из `.env`. В metadata попадают:

| Поле секрета | Переменные `.env` |
|--------------|-------------------|
| `MongoCS` | `KBT_MONGO_HOST`, `TASKER_DB_USER`, `TASKER_DB_PASSWORD`, `TASKER_DB_NAME` |
| `MinioHost` | `KBT_MINIO_HOST` |
| `MinioUser`, `MinioPass` | `MINIO_ROOT_USER`, `MINIO_ROOT_PASSWORD` |

Адреса не вписывают в JSON вручную.
...
Чек-лист до первого `vault-init`:

| Что проверить | Адрес |
|---------------|--------|
| MongoDB принимает соединение | `KBT_MONGO_HOST` |
| MinIO отвечает | `KBT_MINIO_HOST` |
| Vault выдаёт токен | Vault запущен, `API_VAULT_URL` |
| Tasker читает этот же Vault | `API_VAULT_URL` на сервере Tasker |

Если проверка не прошла, секрет всё равно может создаться — уже с неверным адресом. Повторный запуск `vault-init` существующий секрет не перезаписывает. Смена `.env` после первого запуска сама секрет не обновляет.
...
Модель сертификатов — п. 2.5: Let's Encrypt, свои файлы, nginx на хосте или внешний балансировщик.

На сервере Nginx с контейнером из `compose.nginx.yml`:
...
2. Сертификаты по выбранной модели (п. 2.5). Для Let's Encrypt порты 80 и 443 свободны. Для своих файлов Certbot не нужен.
...
Свой nginx на хосте: `compose.nginx.yml` не запускайте. Проксируйте на AppServer `:8080`, DocumentServer `:8880`, MinIO `:9000`. Примеры — `multi-server-install/nginx/examples/host/`. Сертификат живёт в конфигурации хостового nginx.

Внешний балансировщик: если пользователи не обращаются к портам 80 и 443 этого хоста, контейнер nginx и Certbot не запускайте.
